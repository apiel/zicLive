import { clear, drawFilledRect, drawRect, setColor } from 'zic_node_ui';
...
export async function sequencerEditView() {
    ...
    let row = 0;
    drawField(`Sequence`, `#${selectedId + 1}`, row++, {
        edit: (direction) => {
    ...
    drawField(`Track`, track.name, row++, {
        edit: (direction) => {
    ...
    drawField(`Patch`, patch.name, row++, {
        edit: (direction) => {
    ...
    drawField(`Pattern`, '#' + patternId.toString().padStart(3, '0'), row++, {
        edit: (direction) => {
    ...
    drawField(`Detune`, detune < 0 ? detune.toString() : `+${detune}` + ' semitones', row++, {
        edit: (direction) => {
    ...
    drawField(`Repeat`, `x${repeat}${repeat === 0 ? ' infinite' : ' times'}`, row++, {
        edit: (direction) => {
    ...
    drawField(`Next`, nextSequenceId
        ? `${nextSequenceId + 1} ${getPatch(track.engine, patchId).name}`
        : `---`, row++, {
        edit: (direction) => {
            if (direction !== 0) {
                const ids = sequences
...
export async function sequencerEditEventHandler(events) {
